...
class GroupedGemmRunner(torch.nn.Module):
    ...
    def forward(
        ...
        if self.use_flashinfer:
            ...
            c = grouped_gemm_triton_no_dispose(
                a,
                b,
                c,
                batch_size,
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        if self.quant_config.activation_scheme == "static":
            ...
            w1a_input_scale = torch.nn.Parameter(
                torch.ones(num_experts_per_partition, dtype=torch.float32),
                requires_grad=False,
            )
            layer.register_parameter("w1a_input_scale", w1a_input_scale)
            set_weight_attrs(w1a_input_scale, extra_weight_attrs)

            w3a_input_scale = torch.nn.Parameter(
                torch.ones(num_experts_per_partition, dtype=torch.float32),
                requires_grad=False,
            )
            layer.register_parameter("w3a_input_scale", w3a_input_scale)
            set_weight_attrs(w3a_input_scale, extra_weight_attrs)
            ...
            layer.w1a_input_scale = None
            layer.w3a_input_scale = None
